...
import { Task, UserProfile, DailyPlan, SheetsSyncConfig, Subtask, formatDuration, calculateTaskRisk, getDerivedStatus } from "./types";
...
import { TaskInsightsWorkspace } from "./components/TaskInsightsWorkspace";
import { TimelineModal } from "./components/TimelineModal";
import { syncTasksToSheet } from "./googleSync";
import { sendDailyPlanEmail, sendTaskReminderEmail } from "./googleGmail";
...
// Pre-populated high-craft sample tasks for demo purposes & initial onboarding
const DEFAULT_SAMPLE_TASKS: Task[] = [
  {
    id: "sample-1",
    title: "Draft Pitch Deck for Venture Capital Round",
    description: "Incorporate Q2 growth milestones, financial projections, and clear product roadmap slides for upcoming meetings.",
    ...
    userId: "sample",
    subtasks: [
      { id: "sub-1", title: "Refine value proposition slide", status: "completed", estimatedHours: 2 },
      { id: "sub-2", title: "Verify financial model & operating margins", status: "pending", estimatedHours: 4 },
      { id: "sub-3", title: "Design mockup assets for core dashboard overview", status: "pending", estimatedHours: 3 },
      { id: "sub-4", title: "Record a 5-minute video walkthrough draft", status: "pending", estimatedHours: 3 },
    ],
    executionStrategy: "Synthesize the financials early. Visual assets can be compiled as slides are populated. Prioritize the operating margins slide since it requires deep focus and has highest peer dependency.",
    riskAnalysis: {
      riskLevel: "medium",
      probability: 45,
      reasoning: "Task requires 12 hours of total focus over 3 days. While doable, any further delays on financials will risk missing the deadline.",
    },
  ...
  {
    id: "sample-2",
    title: "Compile Chemistry Lab Report & Analysis",
    description: "Document test methodologies, summarize findings, draw chemical molecular formulas, and include lab peer reviews.",
    ...
    userId: "sample",
    subtasks: [
      { id: "sub-2-1", title: "Write introduction & materials section", status: "pending", estimatedHours: 1 },
      { id: "sub-2-2", title: "Import testing data plots & charts", status: "pending", estimatedHours: 2 },
      { id: "sub-2-3", title: "Formulate summary hypothesis", status: "pending", estimatedHours: 2 },
    ],
    executionStrategy: "Activate immediate emergency actions. You have 4 hours left for a 5-hour task! Reduce report scope, write the analysis directly, and reference previous lab methodologies to bypass outline drafting.",
    riskAnalysis: {
      riskLevel: "high",
      probability: 85,
      reasoning: "The estimated effort (5 hours) exceeds the time remaining (4 hours). Immediate, high-intensity rescue protocol is required to complete this task on time.",
    },
    rescuePlan: {
      emergencyPlan: "Cut unnecessary report decorative formatting. Write scientific conclusions directly from crude charts and compile peer citations concurrently. Turn off notifications and set 45-minute sprints.",
      timeBlockedSchedule: [
        { timeSlot: "Next 15 minutes", action: "Open document, paste testing templates, insert raw charts." },
        { timeSlot: "Hour 1 - Hour 2", action: "Draft data analysis sections, skipping molecular rendering." },
        { timeSlot: "Hour 2 - Hour 3", action: "Complete peer overview citations & introduction." },
        { timeSlot: "Hour 3 - Hour 4", action: "Proofread and export PDF directly." },
      ],
      immediateActions: [
        "Open your Google Doc lab draft right now.",
        "Paste the raw testing telemetry table.",
        "Mute your smartphone and set an alarm for 45 minutes."
      ],
    },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  ...
  {
    id: "sample-3",
    title: "Clean out Gmail newsletter inbox backlog",
    description: "Archive marketing materials, delete spam alerts, and sort receipts under the finance folder.",
    ...
    userId: "sample",
    subtasks: [
      { id: "sub-3-1", title: "Archive newsletter threads", status: "completed", estimatedHours: 1 },
      { id: "sub-3-2", title: "Delete spam receipts", status: "completed", estimatedHours: 1 },
    ],
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
...
function generateFallbackSubtasks(taskTitle: string, effortHours: number): Subtask[] {
  ...
  if (titleLower.includes("deploy") || titleLower.includes("website") || titleLower.includes("web") || titleLower.includes("app")) {
    return [
      { id: "fb-1", title: "Review code and resolve active issues", status: "pending", estimatedHours: stepTime },
      { id: "fb-2", title: "Test production build and staging links", status: "pending", estimatedHours: stepTime },
      { id: "fb-3", title: "Configure hosting environment and variables", status: "pending", estimatedHours: stepTime },
      { id: "fb-4", title: "Deploy application and verify live status", status: "pending", estimatedHours: Math.max(1, effortHours - stepTime * 3) },
    ];
  }
  
  if (titleLower.includes("write") || titleLower.includes("draft") || titleLower.includes("report") || titleLower.includes("document") || titleLower.includes("essay")) {
    return [
      { id: "fb-1", title: "Research requirements and source citations", status: "pending", estimatedHours: stepTime },
      { id: "fb-2", title: "Outline core sections and thesis elements", status: "pending", estimatedHours: stepTime },
      { id: "fb-3", title: "Draft main paragraphs and summaries", status: "pending", estimatedHours: stepTime },
      { id: "fb-4", title: "Proofread and polish final document", status: "pending", estimatedHours: Math.max(1, effortHours - stepTime * 3) },
    ];
  }

  if (titleLower.includes("study") || titleLower.includes("learn") || titleLower.includes("exam") || titleLower.includes("course") || titleLower.includes("test")) {
    return [
      { id: "fb-1", title: "Review syllabus and critical lecture slides", status: "pending", estimatedHours: stepTime },
      { id: "fb-2", title: "Synthesize core concepts and write study guide", status: "pending", estimatedHours: stepTime },
      { id: "fb-3", title: "Complete practice questions or exercises", status: "pending", estimatedHours: stepTime },
      { id: "fb-4", title: "Do a final active recall test of key terms", status: "pending", estimatedHours: Math.max(1, effortHours - stepTime * 3) },
    ];
  }

  return [
    { id: "fb-1", title: "Define requirements & structural plan", status: "pending", estimatedHours: stepTime },
    { id: "fb-2", title: "Gather required resources & details", status: "pending", estimatedHours: stepTime },
    { id: "fb-3", title: "Execute core task steps and deliverables", status: "pending", estimatedHours: stepTime },
    { id: "fb-4", title: "Validate outcomes & perform final review", status: "pending", estimatedHours: Math.max(1, effortHours - stepTime * 3) },
  ];
}
...
export default function App() {
  ...
  const notifications = React.useMemo(() => {
    ...
    const overdueCount = tasks.filter(t => getDerivedStatus(t) === "overdue").length;
    const startingSoonCount = tasks.filter(t => {
      if (getDerivedStatus(t) !== "pending" || !t.startTime) return false;
      const dObj = safeDate(t.startTime);
      return dObj && dObj.getTime() > now && dObj.getTime() < oneHourFromNow;
    }).length;
  ...
  const [rankingExplanations, setRankingExplanations] = useState<Record<string, string>>({});
  const [taskToDelete, setTaskToDelete] = useState<string | null>(null);
  const [deleteError, setDeleteError] = useState<string | null>(null);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [isDeleteDropdownOpen, setIsDeleteDropdownOpen] = useState(false);
  const [deleteStep, setDeleteStep] = useState<"options" | "confirm-all" | "confirm-completed">("options");
  ...
  const [completedTaskCelebration, setCompletedTaskCelebration] = useState<string | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (detailsPanelRef.current && detailsPanelRef.current.contains(e.target as Node)) {
        return;
      }
  ...
  useEffect(() => {
    // Check if user is in local mode initially
    const cachedLocal = localStorage.getItem("local_mode") === "true";
    ...
    const unsubscribe = initAuth(
      ...
      () => {
        setNeedsAuth(true);
        // Load default samples to guarantee instant display during unauthenticated/local mode
        const cached = localStorage.getItem("local_tasks");
        if (cached) {
          setTasks(JSON.parse(cached));
        } else {
          setTasks(DEFAULT_SAMPLE_TASKS);
          localStorage.setItem("local_tasks", JSON.stringify(DEFAULT_SAMPLE_TASKS));
        }
        
        const cachedProfile = localStorage.getItem("demo_profile");
        if (cachedProfile) {
          setUserProfile(JSON.parse(cachedProfile));
        } else {
          setUserProfile({
            userId: "demo",
            email: "demo@nowornever.ai",
  ...
  const loadUserData = async (userId: string, email: string) => {
    try {
      ...
      console.error("Failed to sync cloud Firestore, falling back to local database:", err);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        await loadUserData(result.user.uid, result.user.email || "");
      }
    } catch (err) {
  ...
  const handleGenerateBreakdownLater = async (task: Task) => {
    ...
    try {
      ...
      if (breakdownRes.ok) {
        ...
        bdStrategy = "Establish continuous focus. Break down elements and proceed sequentially to guarantee delivery.";
        isFallbackBd = true;
      }
      ...
      console.warn("Failed to generate breakdown later, falling back to rule-based subtasks:", err);
      // Fallback
      const fallbackSub = generateFallbackSubtasks(task.title, task.effortHours);
      const fallbackStrategy = "Establish continuous focus. Break down elements and proceed sequentially to guarantee delivery.";
  ...
  const handleTaskSubmit = async (taskData: {
    ...
    let subtasks: Subtask[] = editTask ? editTask.subtasks : generateFallbackSubtasks(taskData.title, taskData.effortHours);
    let executionStrategy = editTask ? (editTask.executionStrategy || "") : "Establish continuous focus. Break down elements and proceed sequentially to guarantee delivery.";
    let breakdownGenerated = editTask ? (editTask.breakdownGenerated ?? false) : true;
    let riskAnalysis = editTask ? (editTask.riskAnalysis || { riskLevel: "low" as const, probability: 10, reasoning: "Local calculation" }) : {
      riskLevel: "medium" as const,
      probability: 50,
      reasoning: "Steady execution block active. Track subtask checklist status updates."
    };
    let aiSummary = editTask ? (editTask.aiSummary || "") : `Action-focused track regarding ${taskData.title}.`;
    let aiPriority: "low" | "moderate" | "high" = editTask ? (editTask.aiPriority || "moderate") : (taskData.priority === "urgent" || taskData.priority === "high" ? "high" : taskData.priority === "low" ? "low" : "moderate");
    let isFallbackBd = editTask ? (editTask.breakdownIsFallback ?? false) : true;
    ...
    if (trimmedCategory) {
      const defaultMatch = DEFAULT_CATEGORIES.find(c => c.toLowerCase() === trimmedCategory.toLowerCase());
      const currentCustom = userProfile?.categories || [];
      const customMatch = currentCustom.find(c => c.toLowerCase() === trimmedCategory.toLowerCase());
      ...
      if (defaultMatch) {
        ...
        if (user) {
          saveUserProfile(user.uid, user.email || "", { categories: updatedCategories }).catch(err => {
    ...
    const parsedDeadline = taskData.deadline ? (safeDate(taskData.deadline)?.toISOString() || "") : "";
    const parsedStartTime = taskData.startTime ? (safeDate(taskData.startTime)?.toISOString() || "") : "";
    ...
    const newTask: Task = {
      ...
      breakdownGenerated,
      breakdownIsFallback: isFallbackBd,
      reminderSent: editTask ? editTask.reminderSent : false,
      emailReminder: editTask ? editTask.emailReminder : undefined,
      reminderEnabled: editTask ? editTask.reminderEnabled : undefined,
      reminderOffset: editTask ? editTask.reminderOffset : undefined,
      reminderUnit: editTask ? editTask.reminderUnit : undefined,
      rescuePlan,
      coachAdvice,
      ...
      expectedCompletion: parsedStartTime 
        ? new Date(new Date(parsedStartTime).getTime() + taskData.effortHours * 3600 * 1000).toISOString()
        : "",
      ...
      if (isNew || hasChanged) {
        try {
          ...
          if (analyzeRes.ok) {
            ...
            if (isNew || !editTask?.subtasks || editTask.subtasks.length === 0) {
            ...
            setTasks((prevTasks) => {
              const updated = prevTasks.map((t) => (t.id === taskId ? finalTask : t));
              setSelectedTask((selected) => (selected && selected.id === taskId ? finalTask : selected));
              if (user) {
  ...
  const handleToggleSubtask = async (taskId: string, subtaskId: string) => {
    const updatedTasks = tasks.map((task) => {
      if (task.id === taskId) {
        const updatedSubtasks = task.subtasks.map((sub) =>
          sub.id === subtaskId ? { ...sub, status: sub.status === "completed" ? ("pending" as const) : ("completed" as const) } : sub
        );
    ...
    if (updatedSelected) {
      ...
      try {
        const riskRes = await fetch("/api/gemini/risk-analysis", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ task: updatedSelected, currentTime: new Date().toISOString() }),
        });
        if (riskRes.ok) {
          const riskAnalysis = await riskRes.json();
          const taskWithRisk = { ...updatedSelected, riskAnalysis };
          
          setTasks((prev) => prev.map((t) => t.id === taskId ? taskWithRisk : t));
          setSelectedTask(taskWithRisk);

          if (user) {
            await updateExistingTask(user.uid, taskId, { subtasks: updatedSelected.subtasks, riskAnalysis });
          } else {
            localStorage.setItem("local_tasks", JSON.stringify(updatedTasks.map((t) => t.id === taskId ? taskWithRisk : t)));
          }
    ...
    if (user) {
      ...
      if (taskObj) {
        await updateExistingTask(user.uid, taskId, { subtasks: taskObj.subtasks });
      }
    } else {
      localStorage.setItem("local_tasks", JSON.stringify(updatedTasks));
    }
  ...
  const handleTaskComplete = async (taskId: string) => {
    ...
    const updatedTasks = tasks.map((task) => {
      if (task.id === taskId) {
        return { ...task, status: nextStatus as "completed" | "pending", updatedAt: new Date().toISOString() };
      }
      return task;
    });
    ...
    if (userProfile) {
      const currentScore = userProfile.productivityScore !== undefined ? userProfile.productivityScore : 85;
      ...
      if (updatedDailyPlan) {
        const updatedSchedule = updatedDailyPlan.suggestedSchedule.map((item) => {
          if (
            ...
            if (nextStatus === "completed") {
              return {
                ...item,
                taskTitle: item.taskTitle.startsWith("✓ ") ? item.taskTitle : `✓ ${item.taskTitle}`,
                action: item.action.startsWith("✓ ") ? item.action : `✓ Completed successfully! ${item.action.replace(/^•\s*/, "")}`,
              };
            } else {
              return {
                ...item,
                taskTitle: item.taskTitle.replace(/^✓\s*/, ""),
                action: item.action.replace(/^✓\s*Completed successfully!\s*/, "• "),
              };
            }
          }
          return item;
      ...
      (async () => {
        ...
        if (user) {
          ...
          if (!backendFailed && updatedDailyPlan) {
            try {
              await saveUserProfile(user.uid, user.email || "", { dailyPlan: updatedDailyPlan });
            } catch (err) {
              console.error("Firestore user profile dailyPlan update failed:", err);
              backendFailed = true;
            }
          }
        }
        ...
        if (backendFailed) {
          ...
          if (user) {
            localStorage.setItem(`tasks_${user.uid}`, JSON.stringify(originalTasks));
            if (originalUserProfile?.dailyPlan) {
              saveUserProfile(user.uid, user.email || "", { dailyPlan: originalUserProfile.dailyPlan }).catch(e => {
            ...
            if (originalUserProfile) {
              localStorage.setItem("demo_profile", JSON.stringify(originalUserProfile));
            }
          }

          setToast({
            message: "Failed to sync task deletion with database. Task restored.",
            type: "error"
          });
          ...
          try {
            ...
            console.error("Sheets sync background failed during deletion:", err);
          }
        }
      })();
    } catch (err) {
    ...
    try {
      ...
      if (user) {
        try {
          ...
          const deletePromises = originalTasks.map((t) => deleteExistingTask(user.uid, t.id));
  ...
  const deleteCompletedTasks = async () => {
    try {
      ...
      if (userProfile?.dailyPlan) {
        ...
        const filteredPriorities = userProfile.dailyPlan.priorities.filter(
          (p) => !completedIds.includes(p) && !completedTitles.includes(p.toLowerCase())
        );

        const filteredSchedule = userProfile.dailyPlan.suggestedSchedule.filter((item) => {
          const itemTitleClean = item.taskTitle.replace(/^✓\s*/, "").toLowerCase();
      ...
      if (user) {
        try {
          ...
          const deletePromises = completedTasks.map((t) => deleteExistingTask(user.uid, t.id));
          await Promise.all(deletePromises);

          localStorage.setItem(`tasks_${user.uid}`, JSON.stringify(remainingTasks));
        } catch (err) {
  ...
  const handleRankTasks = async () => {
    ...
    if (activeTasks.length <= 1) {
      alert("Please add at least 2 active tasks to perform AI Priority Ranking.");
      return;
    }
    ...
    try {
      ...
      alert("AI Coach has prioritized and re-ranked your active task track! Check the ranked order and details.");
    } catch (err) {
      console.error(err);
  ...
  const handleUpdateSyncConfig = async (config: SheetsSyncConfig) => {
    ...
    await saveUserProfile(user.uid, user.email || "", { sheetsSyncConfig: config });
  };
  ...
  const handleAutoSheetsSync = async (updatedTasks: Task[]) => {
    ...
    if (user && config?.spreadsheetId && config.isEnabled && accessToken) {
      try {
        ...
        await saveUserProfile(user.uid, user.email || "", { sheetsSyncConfig: updatedConfig });
      } catch (err) {
  ...
  const handleUpdateTaskFromRescue = async (updatedTask: Task) => {
    const updatedTasks = tasks.map((t) => (t.id === updatedTask.id ? updatedTask : t));
    setTasks(updatedTasks);
    setSelectedTask(updatedTask);

    if (user) {
      await updateExistingTask(user.uid, updatedTask.id, { rescuePlan: updatedTask.rescuePlan });
    } else {
      localStorage.setItem("local_tasks", JSON.stringify(updatedTasks));
    }
  };

  const handleSaveDailyCoachPlan = async (plan: DailyPlan) => {
    if (user) {
      const updatedProfile = {
        ...userProfile,
        dailyPlan: plan,
      ...
      setUserProfile(updatedProfile);
      await saveUserProfile(user.uid, user.email || "", { dailyPlan: plan });
    } else {
      const mockProfile = {
        userId: "demo",
        email: "demo@nowornever.ai",
  ...
  const uniqueCustomCategories = customCategories.filter(
    (cat) => !DEFAULT_CATEGORIES.some((d) => d.toLowerCase() === cat.toLowerCase())
  );
  const allAvailableCategories = [...DEFAULT_CATEGORIES, ...uniqueCustomCategories];
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    if (categoryFilter !== "all") {
      if ((task.category || "").toLowerCase() !== categoryFilter.toLowerCase()) return false;
    }
  ...
  const isAnyFilterActive = searchQuery.trim() !== "" || priorityFilter !== "all" || statusFilter !== "all" || categoryFilter !== "all" || sortBy !== "default";
  ...
  return (
    <div className="min-h-screen bg-surface-secondary text-text-primary font-sans flex flex-col selection:bg-accent-primary/25 selection:text-text-primary">
            ...
            <div className="relative w-full max-w-sm text-center space-y-8">
              ...
              <div className="space-y-3">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-bg-surface rounded-full border border-border-custom shadow-sm text-accent-primary">
                  ...
                  <span className="text-[10px] font-black tracking-widest uppercase">Smart Task Planner</span>
                </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#EA4335" d="M12.5 12.22c0-.66-.06-1.3-.17-1.9H7v3.58h3.21c-.14.75-.56 1.38-1.18 1.8v2.32h2.24c1.31-1.21 2.06-3 2.06-5.12z"/><path fill="#4285F4" d="M7 12.22c0-.66.06-1.3.17-1.9H3.77c-.11.6-.17 1.24-.17 1.9s.06 1.3.17 1.9h3.4c-.11-.6-.17-1.24-.17-1.9z"/><path fill="#FBBC05" d="M12.33 10.32c.11-.6.17-1.24.17-1.9s-.06-1.3-.17-1.9H7v3.58h5.33z"/><path fill="#34A853" d="M12.33 13.82c-.11.6-.17 1.24-.17 1.9s.06 1.3.17 1.9h3.4c.66-.61 1.15-1.36 1.41-2.2-.42-1.32-1.18-2.52-2.24-3.52l-2.57 1.82z"/></svg>
                    Continue with Google
                  </button>
                  <p className="text-[10px] text-text-muted font-bold">Sync your tasks securely across devices.</p>
                </div>
                ...
                <div className="w-full">
                  <button
                    onClick={() => { setNameModalAction("local"); setIsNameModalOpen(true); }}
                  ...
                  >
                    Continue Locally
                    <p className="text-[9px] text-text-muted mt-0.5 font-medium">Tasks stay on device.</p>
                  </button>
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        <header className="bg-bg-surface border-b border-border-custom sticky top-0 z-40 px-6 md:px-8 py-3 flex items-center justify-between shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-black text-text-primary uppercase tracking-tight">Now or Never</h1>
            </div>
            {localMode && (
          ...
          {/* User Controls */}
          <div className="flex items-center gap-3">
            <button onClick={toggleTheme} className="p-2 text-text-secondary hover:text-text-primary rounded-xl transition-colors">
            ...
            <div className="hidden md:block relative group">
              ...
              <AnimatePresence>
                {isSheetsSyncOpen && tasks.length > 0 && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setIsSheetsSyncOpen(false)} />
                    <motion.div
                      ...
                      <div className="p-1 max-h-[450px] overflow-y-auto custom-scrollbar bg-bg-primary/30">
                        {(!user || localMode) ? (
                          <div className="p-6 text-center space-y-4 bg-bg-surface">
                            ...
                            <div className="space-y-1">
                              <h5 className="text-xs font-bold text-text-primary">Google Account Required</h5>
                              <p className="text-[11px] text-text-secondary leading-relaxed max-w-xs mx-auto">
                                Active login is required to connect, create, and synchronize your tasks to Google Sheets.
                              </p>
                            </div>
                            <button
                              onClick={() => {
                  ...
                  <>
                    ...
                    >
                      <div className="flex items-center justify-between pb-2 border-b border-border-custom">
                        <h4 className="text-[10px] font-black text-text-muted uppercase tracking-widest">Notifications</h4>
                        <button onClick={() => setIsNotificationOpen(false)} className="text-text-muted hover:text-text-primary">
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                        ...
                        {notifications.length > 0 ? (
                          notifications.map((n) => (
                            <div key={n.id} className={`p-3 rounded-xl border flex gap-3 ${
                              n.type === 'danger' ? 'bg-status-danger/10 border-status-danger/20 text-status-danger' :
            ...
            <div className="flex items-center gap-3 border-l border-border-custom pl-3 ml-1">
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  {user?.photoURL ? (
                    <img src={user.photoURL} alt={user.displayName} className="w-8 h-8 rounded-full border border-border-custom" />
                  ) : (
                  ...
                  <div className="hidden md:block text-left">
                    <p className="text-[11px] font-black text-text-primary leading-none">{user?.displayName || "User"}</p>
                    <p className="text-[9px] font-bold text-text-muted mt-0.5 leading-none">Account</p>
                  </div>
                </button>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-5 md:p-8 space-y-6 sm:space-y-8 md:space-y-10">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <h1 className="text-[9px] sm:text-[10px] font-black text-accent-primary uppercase tracking-[0.3em] mb-1">NOW OR NEVER</h1>
              <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-text-primary tracking-tighter leading-tight">Focus. Plan. Achieve.</h2>
            </div>
            <StreakWidget tasks={tasks} />
          </div>
          <AnalyticsPanel 
            tasks={tasks} 
            userName={user?.displayName ? user.displayName.trim().split(/\s+/)[0] : (storedName || undefined)}
        ...
        <div id="all-tasks-section" className="space-y-4 w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <h3 id="active-track-list-header" className="text-sm font-black text-text-primary uppercase tracking-wider">
            ...
            <div className="flex items-center gap-2.5 shrink-0">
              <div className="relative">
                <button
                  ...
                  title={tasks.length === 0 ? "No tasks available." : undefined}
                >
                  <Trash2 className="w-4 h-4" /> Delete Tasks <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isDeleteDropdownOpen ? "rotate-180" : ""}`} />
                </button>

                <AnimatePresence>
                  {isDeleteDropdownOpen && tasks.length > 0 && (
                    <>
                      <div className="fixed inset-0 z-40" onClick={() => setIsDeleteDropdownOpen(false)} />
                      <motion.div
                        initial={{ opacity: 0, scale: 0.95, y: 10 }}
                        animate={{ opacity: 1, scale: 1, y: 0 }}
                        exit={{ opacity: 0, scale: 0.95, y: 10 }}
          ...
          <div className="flex flex-col lg:flex-row gap-3 bg-bg-surface p-3 rounded-2xl border border-border-custom shadow-sm text-xs w-full">
            ...
            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 w-full lg:w-auto justify-start">
              ...
              <div className="flex items-center gap-1.5 flex-1 sm:flex-initial min-w-[110px] sm:min-w-0">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-wider shrink-0 hidden sm:inline">Priority:</span>
                <select
              ...
              {/* Status Filter */}
              <div className="flex items-center gap-1.5 flex-1 sm:flex-initial min-w-[110px] sm:min-w-0">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-wider shrink-0 hidden sm:inline">Status:</span>
                <select
                  value={statusFilter}
              ...
              {/* Category Filter */}
              <div className="flex items-center gap-1.5 flex-1 sm:flex-initial min-w-[110px] sm:min-w-0">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-wider shrink-0 hidden sm:inline">Category:</span>
                <select
              ...
              {/* Sort Filter */}
              <div className="flex items-center gap-1.5 flex-1 sm:flex-initial min-w-[110px] sm:min-w-0">
                <span className="text-[10px] font-black text-text-muted uppercase tracking-wider shrink-0 hidden sm:inline">Sort By:</span>
                <select
                  value={sortBy}
              ...
              >
                ...
                <span className="uppercase tracking-wider font-black">Clear Filters</span>
              </button>
            </div>
          </div>

          {/* Active Filter Info Banner */}
          {(statusFilter !== "all" || priorityFilter !== "all" || categoryFilter !== "all" || sortBy !== "default") && (
            <div className="flex items-center justify-between bg-accent-primary/5 border border-accent-primary/15 rounded-xl px-4 py-3 text-xs">
              <div className="flex items-center gap-2 text-text-secondary">
                ...
                <span className="font-bold">
                  {statusFilter === "completed" && "Showing Completed Tasks"}
                  {statusFilter === "pending" && sortBy !== "deadline" && "Showing Pending Tasks"}
          ...
          {/* Cards Grid */}
          <motion.div 
            animate={highlightTasks ? { boxShadow: "0 0 24px 4px rgba(99, 102, 241, 0.2)" } : {}}
          ...
          >
            ...
            {sortedTasks.length === 0 && (
              <div className="col-span-full text-center py-16 bg-bg-surface rounded-2xl border border-dashed border-border-custom text-text-muted text-xs">
                ...
                {searchQuery ? (
                  <div className="space-y-3">
                    <p className="font-bold text-text-secondary">No tasks match your search.</p>
                    <button
                  ...
                  <div className="space-y-3">
                    <p className="font-bold text-text-secondary">No tasks found in this category.</p>
                    <button
                  ...
                  <p>No matching objectives found. Modify filters or add a new objective.</p>
                )}
              </div>
            )}
          </motion.div>
        ...
        {tasks.length > 0 && (
          <motion.div
            ref={detailsPanelRef}
            animate={highlightSelectedTask ? { scale: [1, 1.015, 1], boxShadow: "0 0 24px 6px rgba(99, 102, 241, 0.3)" } : {}}
        ...
        {isDeleteModalOpen && (
          ...
          >
            <motion.div
              ...
              className="bg-bg-surface rounded-3xl border border-border-custom shadow-2xl p-6 max-w-sm sm:max-w-md w-full space-y-5"
            >
              {deleteStep === "options" && (
                <>
                  <div className="flex items-center justify-between border-b border-border-custom/50 pb-3">
                    <div className="flex items-center gap-3 text-status-danger">
                      ...
                      <h3 className="text-base font-black tracking-tight text-text-primary">Delete Tasks</h3>
                    </div>
                    <button
                  ...
                  <div className="space-y-3.5">
                    ...
                    <div className="space-y-2.5">
                      ...
                      <button
                        ...
                        className="w-full text-left p-3.5 rounded-2xl border border-border-custom hover:border-status-danger/30 hover:bg-status-danger/5 transition-all group flex items-start gap-3 cursor-pointer"
                      >
                        <div className="p-1.5 bg-status-danger/10 rounded-lg group-hover:bg-status-danger/20 transition-all shrink-0">
                      ...
                      <div className="relative group/tooltip">
                        <button
                          disabled={!tasks.some((t) => t.status === "completed")}
                          onClick={() => setDeleteStep("confirm-completed")}
                          className={`w-full text-left p-3.5 rounded-2xl border transition-all flex items-start gap-3 ${
                            tasks.some((t) => t.status === "completed")
                              ? "border-border-custom hover:border-status-danger/30 hover:bg-status-danger/5 cursor-pointer group"
                              : "border-border-custom/50 bg-bg-primary/20 opacity-40 cursor-not-allowed"
                          }`}
                        >
                          <div className={`p-1.5 rounded-lg shrink-0 ${
                            tasks.some((t) => t.status === "completed")
                            ...
                            <CheckSquare className="w-4 h-4 text-status-danger" />
                          </div>
                          <div>
                            <h4 className={`text-xs font-extrabold ${
                              tasks.some((t) => t.status === "completed")
                            ...
                            <p className="text-[11px] text-text-muted mt-0.5 leading-relaxed">
                              Remove only tasks that have been completed.
                            </p>
                          </div>
                        </button>
                        {!tasks.some((t) => t.status === "completed") && (
                          <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-max max-w-[250px] bg-bg-surface border border-border-custom px-3 py-1.5 rounded-xl shadow-xl opacity-0 invisible group-hover/tooltip:opacity-100 group-hover/tooltip:visible transition-all duration-200 z-50 text-center text-[10px] font-bold text-text-primary pointer-events-none">
                            There are no completed tasks to delete.
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2.5 pt-1">
                    <button
                      ...
                      className="w-full px-4 py-2.5 bg-bg-hover hover:bg-border-custom text-text-secondary text-xs font-bold rounded-2xl transition-colors cursor-pointer text-center"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              {deleteStep === "confirm-all" && (
                <>
                  <div className="flex items-center gap-3 text-status-danger">
                    ...
                    <h3 className="text-base font-black tracking-tight text-text-primary">Delete All Tasks?</h3>
                  </div>

                  <div className="space-y-2">
                    <p className="text-xs text-text-secondary leading-relaxed font-bold">
                      This action cannot be undone. All tasks, timelines, subtasks, streak calculations, and related task data will be permanently removed.
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 px-4 py-2.5 bg-status-danger hover:bg-status-danger/90 text-white text-xs font-bold rounded-2xl transition-all cursor-pointer shadow-md shadow-status-danger/10"
                    >
                      Delete All
                    </button>
                  </div>
              ...
              {deleteStep === "confirm-completed" && (
                <>
                  <div className="flex items-center gap-3 text-status-danger">
                    <div className="p-2 bg-status-danger/10 rounded-xl">
                      <AlertTriangle className="w-5 h-5 text-status-danger" />
                    </div>
                    <h3 className="text-base font-black tracking-tight text-text-primary">Delete Completed Tasks?</h3>
                  </div>

                  <div className="space-y-2">
                    <p className="text-xs text-text-secondary leading-relaxed font-bold">
                      Only completed tasks will be permanently removed. Pending and overdue tasks will remain.
                    </p>
                  </div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 text-status-danger">
                ...
                <h3 className="text-base font-black tracking-tight text-text-primary">Delete Task</h3>
              </div>
        ...
        {completedTaskCelebration && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-black text-status-success uppercase tracking-[0.2em] block">Sprint Accomplished!</span>
                <h3 className="text-xl font-black text-text-primary tracking-tight leading-tight">
                  {completedTaskCelebration}
                </h3>
                <p className="text-xs text-text-secondary leading-relaxed max-w-xs mx-auto">
                  Outstanding job! You've successfully conquered this task before the deadline. Your productivity score just got updated.
                </p>
              </div>
              ...
              {userProfile && (
                <div className="p-4 bg-bg-primary rounded-2xl border border-border-custom flex items-center justify-between">
                  <span className="text-[10px] font-black text-text-muted uppercase tracking-wider">Productivity Level</span>
      ...
      <footer className="bg-bg-surface border-t border-border-custom py-6 text-center text-[11px] text-text-muted mt-12">
        <p>&copy; {new Date().getFullYear()} Now or Never. An intelligent full-stack productivity framework.</p>
      </footer>
      </div>
    </div>
  );
